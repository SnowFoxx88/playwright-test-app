...
export class DatePickerPage extends HelperBase{
  ...
  async selectDatepickerWithRangeFromToday(startDayFromToday: number, endDayFromToday: number){
    ...
    const tDateToAssertEnd = await this.selectDateInTheCalender(endDayFromToday)

    await expect(calendarInputField).toHaveValue(`${DateToAssertStart} - ${tDateToAssertEnd}`)
